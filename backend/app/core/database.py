...
def init_db():
    ...
    c.execute("CREATE INDEX IF NOT EXISTS idx_achados_status ON achados(status)")
    c.execute("CREATE INDEX IF NOT EXISTS idx_achados_status_fluxo ON achados(status_fluxo)")
    c.execute("CREATE INDEX IF NOT EXISTS idx_produtos_achado ON produtos(achado_id)")
    c.execute("CREATE INDEX IF NOT EXISTS idx_engagement_meta_id ON engagement_logs(meta_id)")
...
def optimize_db():
    """Realiza manutenção no banco de dados (VACUUM e ANALYZE)."""
    conn = get_connection()
    try:
        conn.execute("VACUUM;")
        conn.execute("ANALYZE;")
        print("[Database] Otimização concluída com sucesso.")
    except Exception as e:
        print(f"[Database] Erro ao otimizar: {e}")
    finally:
        conn.close()
